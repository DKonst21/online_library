...
Данный репозиторий имеет все необходимые файлы для запуска библиотеки оффлайн.
Достаточно скачать данные из папок:
```shell
/media
/pages
/static
...
## Что за проект?
Данная проект создан в образовательных целях на онлайн-курсе для веб-разработчиков [dvmn.org](https://dvmn.org/).
Пример результата можно увидеть по ссылке: [Github Pages проекта](https://DKonst21.github.io/online_library/)
...
Для запуска достаточно ввести:
```
python render_website.py --json_filepath my_json_file.json
```
`--json_filepath` - необязательный параметр, принимает расположение файла с данными по книгам
...
После запуска сервера, библиотека будет доступна по адресу: [http://127.0.0.1:5500/pages/index1.html](http://127.0.0.1:5500/pages/index1.html)